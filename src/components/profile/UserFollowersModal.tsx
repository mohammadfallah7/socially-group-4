...
export const UserFollowersModal = ({
  ...
  const { data: myFollowings } = useUserFollowings(session?.user.id);
  ...
  useEffect(() => {
    if (followers) {
      ...
      const myFollowingIds = new Set(
        myFollowings?.map((item: any) => (item.following || item).id) || []
      );

      followers.forEach((item: any) => {
        const targetUser = item.follower || item;
        const isFollowing =
          myFollowingIds.has(targetUser.id) ||
          item.isFollowing ||
  ...
  }, [followers, myFollowings, session?.user.id]);
            ...
            <ScrollArea className="h-full pr-3">
              <div className="space-y-4">
                {followers.map((item: any) => {
                  ...
                  const isFollowing = followingMap[targetUser.id] ?? false;
